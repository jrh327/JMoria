...
public class Desc {
	...
	String[] titles = new String[Constants.MAX_TITLES];
	
	private static Desc instance;
	private Desc() { }
	public static Desc getInstance() {
		if (instance == null) {
			instance = new Desc();
			instance.init();
		}
	 ...
	 */
	public boolean is_a_vowel(char ch)  {
		switch(ch)
		{
	...
	public void magic_init() {
		...
		for (h = 0; h < Constants.MAX_TITLES; h++) {
			...
			k = m1.randint(2) + 1;
			for (i = 0; i < k; i++) {
				for (j = m1.randint(2); j > 0; j--) {
					string = string.concat(Tables.syllables[m1.randint(Constants.MAX_SYLLABLES) - 1]);
				}
				if (i < k - 1) {
	 ...
	 * @return Item type
	 */
	public int object_offset(InvenType t_ptr) {
		switch (t_ptr.tval)
		{
	 ...
	 */
	public void known1(InvenType i_ptr) {
		int offset;
		int indexx;
		...
		t.object_ident[offset + indexx] |= Constants.OD_KNOWN1;
		/* clear the tried flag, since it is now known */
		t.object_ident[offset + indexx] &= ~Constants.OD_TRIED;
	}
	 ...
	 */
	public boolean known1_p(InvenType i_ptr) {
		int offset;
		int indexx;
		...
		offset <<= 6;
		indexx = i_ptr.subval & (Constants.ITEM_SINGLE_STACK_MIN - 1);
		return (t.object_ident[offset + indexx] != 0);
	}
	
	/* Remove "Secret" symbol for identity of plusses			*/
	public void known2(InvenType i_ptr) {
		unsample(i_ptr);
		i_ptr.ident |= Constants.ID_KNOWN2;
	}
	
	public boolean known2_p(InvenType i_ptr) {
		return (i_ptr.ident & Constants.ID_KNOWN2) != 0;
	}
	
	public void clear_known2(InvenType i_ptr) {
		i_ptr.ident &= ~Constants.ID_KNOWN2;
	}
	
	public void clear_empty(InvenType i_ptr) {
		i_ptr.ident &= ~Constants.ID_EMPTY;
	}
	
	public void store_bought(InvenType i_ptr) {
		i_ptr.ident |= Constants.ID_STOREBOUGHT;
		known2(i_ptr);
	}
	
	public boolean store_bought_p(InvenType i_ptr) {
		return (i_ptr.ident & Constants.ID_STOREBOUGHT) != 0;
	}
	
	/*	Remove an automatically generated inscription.	-CJS- */
	public void unsample(InvenType i_ptr) {
		int offset;
		int indexx;
	...
	public void sample(InvenType i_ptr) {
		int offset;
		int indexx;
	 ...
	 * when appropriate */
	public void identify(IntPointer item) {
		int i, x1, x2;
		int j;
		InvenType i_ptr, t_ptr;
		
		i_ptr = t.inventory[item.value()];
			...
			if (x2 < Constants.ITEM_SINGLE_STACK_MIN || x2 >= Constants.ITEM_GROUP_MIN) {
				...
				for (i = 0; i < t.inven_ctr; i++) {
					...
					if (t_ptr.tval == x1 && t_ptr.subval == x2 && i != item.value() && (t_ptr.number + i_ptr.number < 256)) {
						...
						t.inventory[item.value()].number += t.inventory[i].number;
						t.inven_ctr--;
						for (j = i; j < t.inven_ctr; j++) {
							t.inventory[j] = t.inventory[j + 1];
						}
						invcopy(t.inventory[j], Constants.OBJ_NOTHING);
					}
				}
			}
		}
	 ...
	 */
	public void unmagic_name(InvenType i_ptr) {
	...
	private final int PLUSSES   = 2;
	private final int LIGHT     = 3;
	private final int FLAGS     = 4;
	private final int Z_PLUSSES = 5;
	...
	public String objdes(InvenType i_ptr, boolean pref) {
		...
		indexx = i_ptr.subval & (Constants.ITEM_SINGLE_STACK_MIN - 1);
		basenm = t.object_list[i_ptr.index].name;
		modstr = "";
		damstr = "";
		p1_use = IGNORED;
		...
		case Constants.TV_DOWN_STAIR:
			out_val = t.object_list[i_ptr.index].name.concat(".");
			return out_val;
		case Constants.TV_STORE_DOOR:
			out_val = String.format("the entrance to the %s.", t.object_list[i_ptr.index].name);
			return out_val;
		default:
		...
		if (append_name) {
			tmp_val = tmp_val.concat(" of ").concat(t.object_list[i_ptr.index].name);
		}
		if (i_ptr.number != 1) {
		...
		if (!pref) {
			...
			if (i_ptr.name2 != Constants.SN_NULL && known2_p(i_ptr)) {
				tmp_val = tmp_val.concat(" ").concat(t.special_names[i_ptr.name2]);
			}
			if (!damstr.equals("")) {
			...
			if ((indexx = object_offset(i_ptr)) >= 0) {
				...
				if ((t.object_ident[indexx] & Constants.OD_TRIED) != 0 && !store_bought_p(i_ptr)) {
	...
	public void desc_charges(int item_val) {
		int rem_num;
		String out_val;
		
		if (known2_p(t.inventory[item_val])) {
			rem_num = t.inventory[item_val].p1;
	...
	public void desc_remain(int item_val) {
