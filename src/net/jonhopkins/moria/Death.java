...
public class Death {
	 ...
	 * @return
	 */
	public static String centerString(String toCenter) {
		StringBuilder centered = new StringBuilder(toCenter.length());
		
		int middle = 15 - toCenter.length() / 2;
		...
		centered.append(' ').append(toCenter).append(' ');
		for (int i = 0; i < middle - 1; i++) {
			centered.append(' ');
		}
...
*/		
	...
	private static void printTomb() {
		...
		String str = String.format("/%s\\,;_          _;,,,;_", centerString(Player.py.misc.name));
		IO.putBuffer(str, 6, 10);
		IO.putBuffer("|               the               |   ___", 7, 9);
		
		String title;
		if (!Variable.isTotalWinner) {
			title = Misc3.getPlayerTitle();
		} else {
			title = "Magnificent";
		}
		str = String.format("| %s |  /   \\", centerString(title));
		IO.putBuffer(str, 8, 9);
		IO.putBuffer("|", 9, 9);
		IO.putBuffer("|  :   :", 9, 43);
		
		if (!Variable.isTotalWinner) {
			title = Player.Class[Player.py.misc.playerClass].title;
		} else if (Player.py.misc.isMale) {
			title = "*King*";
		} else {
			title = "*Queen*";
		}
		str = String.format("| %s | _;,,,;_   ____", centerString(title));
		IO.putBuffer (str, 10, 9);
		str = String.format("Level : %d", Player.py.misc.level);
		str = String.format("| %s |          /    \\", centerString(str));
		...
		str = String.format("| %s |         _;,,,,;_", centerString(str));
		IO.putBuffer(str, 14, 9);
		IO.putBuffer("|            killed by            |", 15, 9);
		
		String causeOfDeath = Variable.diedFrom;
		causeOfDeath += "."; // add a trailing period
		str = String.format("| %s |", centerString (causeOfDeath));
		IO.putBuffer(str, 16, 9);
		causeOfDeath.substring(0, causeOfDeath.length() - 1); // strip off the period
		String day = date();
		str = String.format("| %s |", centerString(day));
	...
	public static int getTotalPoints() {
		int total = Player.py.misc.maxExp + (100 * Player.py.misc.maxDungeonLevel);
	...
	private static void highscores() {
		...
		HighScoreType newEntry = new HighScoreType();
		newEntry.points = getTotalPoints();
		newEntry.birthDate = Variable.birthDate;
		newEntry.uid = 1000000; //(getgid()*1000) + getuid();
		newEntry.maxHitpoints = Player.py.misc.maxHitpoints;
		newEntry.currHitpoints = Player.py.misc.currHitpoints;
		newEntry.dungeonLevel = Variable.dungeonLevel;
		newEntry.level = Player.py.misc.level;
		newEntry.maxDungeonLevel = Player.py.misc.maxDungeonLevel;
		newEntry.sex = (Player.py.misc.isMale ? 'M' : 'F');
		newEntry.race = Player.py.misc.playerRace;
		newEntry.playerClass = Player.py.misc.playerClass;
		newEntry.name = Player.py.misc.name;
		String tmp = Variable.diedFrom;
		...
		newEntry.diedFrom = tmp.substring(i);
...
*/	}
	...
	private static void kingly() {
		...
		Spells.restoreLevel();
		misc.level += Constants.MAX_PLAYER_LEVEL;
		misc.gold += 250000L;
		misc.maxExp += 5000000L;
		misc.currExp = misc.maxExp;
		...
		IO.putBuffer("\"$$       $$$       $$\"", 10, 23);
		final String border = "*#########*#########*";
		IO.putBuffer(border, 11, 24);
		IO.putBuffer(border, 12, 24);
